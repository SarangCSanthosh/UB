...
""")
		# ==========================================================
# 🔹 TOP / BOTTOM LOCATIONS VISUALIZATION (for secondary dataset)
# ==========================================================

st.markdown("###  Question: Where are shipments highest and where are they lagging?")
st.subheader("Top/Bottom Locations Analysis")

LOCATION_COL = "DBF_DEPOT"  # or use "LOCATION" if that's your column name
VOLUME_COL = "VOLUME"

if LOCATION_COL in df_filtered.columns and VOLUME_COL in df_filtered.columns:

    # --- Standardize location/depot names ---
    ...
    # --- Group shipment volume by location ---
    location_volume = (
    ...
    # --- User Controls ---
    choice = st.radio("Select Type", ["Top", "Bottom"], horizontal=True, key="tb_choice_secondary")
    value_type = st.radio("Value Type", ["Absolute", "Percentage"], horizontal=True, key="tb_value_type_secondary")
    n_locations = st.slider("Number of Locations", 5, 25, 10, key="tb_slider_secondary")
    ...
    if value_type == "Percentage":
        ...
        locs[VOLUME_COL] = (locs[VOLUME_COL] / total_volume * 100).round(1)
        x_title = "Share (%)"
    else:
        x_title = "Shipment Volume"
    ...
    fig = px.bar(
        ...
        y=LOCATION_COL,
        orientation="h",
        text=locs[VOLUME_COL].round(1),
        title=f"{choice} {n_locations} Locations by {x_title}",
        color=VOLUME_COL,
    ...
    st.markdown("""
    ### **Insights:**
    - 🏭 **Top Depots** (e.g., Kalaburagi, Hubballi) handle most shipments — core distribution hubs.  
    - ⚙️ **Bottom Depots** show lower activity — possibly seasonal, rural, or underdeveloped markets.  
    - 📈 Shipment concentration among top depots indicates centralized logistics, suggesting potential to diversify load.
    """)
else:
    st.warning(f"Required columns `{LOCATION_COL}` and `{VOLUME_COL}` not found in dataset.")
...
""")
    ...
    with tab5:
        ...
        DEPOT_COORDS = {
            "BIDAR": (17.9100, 77.5199),
            "KALABURAGI": (17.3297, 76.8343),
            "SEDAM": (17.1794, 77.2830),
            "YADHAGIRI": (16.7700, 77.1400),
