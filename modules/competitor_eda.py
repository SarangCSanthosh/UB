...
def run():
    ...
    with tab1:
        ...
        if selected_brand == "All":
            ...
            if granularity == "Percentage":
                y_col = "Percentage"
                text_values = brand_sales[y_col].round(2)
                y_title = "Volume Share (%)"
            else:
                y_col = VOLUME_COL
                text_values = brand_sales[VOLUME_COL].apply(lambda x: f"{x:,.0f}")
                y_title = "Volume"
            fig = px.bar(brand_sales, x="Brand", y=y_col, text=text_values, labels={y_col: y_title}, color="Brand")
            st.dataframe(
                brand_sales.set_index("Brand")[[VOLUME_COL, "Percentage"]],
                formatters={VOLUME_COL: "{:,.0f}".format, "Percentage": "{:.2f}%".format}
            )
        else:
            ...
            if granularity == "Percentage":
                y_col = "Percentage"
                text_values = brand_data[y_col].round(2)
                y_title = "Volume Share (%)"
            else:
                y_col = VOLUME_COL
                text_values = brand_data[VOLUME_COL].apply(lambda x: f"{x:,.0f}")
                y_title = "Volume"
            fig = px.bar(
                brand_data, x="Segment", y=y_col, text=text_values,
                labels={y_col: y_title}, color="Segment"
            )
            st.dataframe(
                brand_data.set_index("Segment")[[VOLUME_COL, "Percentage"]],
                formatters={VOLUME_COL: "{:,.0f}".format, "Percentage": "{:.2f}%".format}
            )
    ...
    with tab2:
        ...
        pack_sales = df.groupby("Segment")[VOLUME_COL].sum().reset_index().sort_values(by=VOLUME_COL, ascending=False)
        ...
        if granularity_pack == "Percentage":
            ...
            text_values = pack_sales["Value"].round(2)
            y_col = "Value"
            y_title = "Volume Share (%)"
        else:
            pack_sales["Value"] = pack_sales[VOLUME_COL]
            text_values = pack_sales[VOLUME_COL].apply(lambda x: f"{x:,.0f}")
            y_col = "Value"
            y_title = "Volume"

        fig_pack = px.bar(pack_sales, x="Segment", y=y_col, text=text_values, title="Pack Size Distribution", color="Segment")
        ...
        st.dataframe(
            pack_sales.set_index("Segment")[[VOLUME_COL, "Value"]],
            formatters={VOLUME_COL: "{:,.0f}".format, "Value": "{:.2f}%".format if granularity_pack=="Percentage" else "{:,.0f}".format}
        )
    ...
    with tab3:
        ...
        fig_packtype = px.pie(pack_type_sales, names="Pack_Type", values=VOLUME_COL, title="Bottle vs Can Volume Distribution", hole=0.4, color="Pack_Type")
        ...
        st.dataframe(pack_type_sales.set_index("Pack_Type")[[VOLUME_COL]], formatters={VOLUME_COL: "{:,.0f}".format})
    ...
    with tab4:
        ...
        if granularity_sku == "Percentage":
            ...
            text_values = sku_data["Value"].round(2)
            y_col = "Value"
            y_title = "Volume Share (%)"
        else:
            sku_data["Value"] = sku_data[VOLUME_COL]
            text_values = sku_data[VOLUME_COL].apply(lambda x: f"{x:,.0f}")
            y_col = "Value"
            y_title = "Volume"

        fig_sku = px.bar(sku_data, x=SKU_COL, y=y_col, text=text_values, title=f"Top {top_n} SKUs")
        ...
        st.dataframe(
            sku_data.set_index(SKU_COL)[[VOLUME_COL, "Value"]],
            formatters={VOLUME_COL: "{:,.0f}".format, "Value": "{:.2f}%".format if granularity_sku=="Percentage" else "{:,.0f}".format}
        )
